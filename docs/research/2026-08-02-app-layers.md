...
- A **deterministic normaliser** converts to integer minor units using the locale grammar and the ISO 4217 exponent, and returns an ambiguity flag when the grammar is undecidable. **Correction (2026-08-03, from the implementation in `packages/extraction`):** `1.299` in a MX receipt was cited here as the ambiguous case and is not. Reading the dot as a decimal point yields 1.299 pesos — fractional cents, not representable on a 2-decimal currency — so grouping is the only arithmetically valid reading and the case is decidable. **The genuinely undecidable case is a 3-decimal currency:** `1,500` KWD is either 1500 KWD (grouped) or 1.500 KWD (decimal), both whole numbers of fils, and nothing in the string chooses. The rule is therefore: flag ambiguity only when *both* readings are arithmetically valid, which happens exactly when the trailing-digit count equals both the exponent and 3.
